...
from langchain_community.embeddings import QianfanEmbeddingsEndpoint    # 调用百度的 Embeddings 模型
import os
...
def get_vectordb(file_path:str=None, persist_path:str=None, embedding = "wenxin",embedding_key:str=None):
    """
    返回向量数据库对象
    输入参数：
    question：
    ...
    embedding：可以使用zhipuai等embedding，不输入该参数则默认使用 百度的 embedding，注意此时api_key不要输错
    """
